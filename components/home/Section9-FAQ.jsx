...
function FAQSection() {
  return (
    <div className="px-2 sm:w-1/2 sm:px-0 sm:mt-32 mt-6 ">
      <p className="text-4xl font-bold sm:text-3xl mb-4">
      ...
      >
        ...
        <AccordionItem value="item-2">
          ...
          <AccordionContent>
            We are preparing for a crowdfunding launch in the coming months this
            year. Sign Up to get notified when the campaign launches.
          </AccordionContent>
        </AccordionItem>
      ...
      <p className="font-medium text-xs sm:text-sm py-1 font sm:bg-transparent bg-white">
        For any more questions, feel free to reach out to our support team
        at&nbsp;
        <a href="mailto:snehan@motsats.com" className="text-blue-500">
          snehan@motsats.com
        </a>
      </p>
    </div>
...
const AccordionTrigger = React.forwardRef(
  ({ children, className, ...props }, forwardedRef) => (
    <Accordion.Header className="flex">
      <Accordion.Trigger
        className={classNames(
          'text-black  hover:bg-neutral-50 group flex h-[45px] flex-1 cursor-default items-center justify-between bg-white px-5 sm:text-base text-sm leading-none border  outline-none',
          className
        )}
        {...props}
        ref={forwardedRef}
...
const AccordionContent = React.forwardRef(
  ({ children, className, ...props }, forwardedRef) => (
    <Accordion.Content
      className={classNames(
        'text-black data-[state=open]:animate-slideDown data-[state=closed]:animate-slideUp overflow-hidden text-sm sm:text-base bg-neutral-200',
        className
      )}
      {...props}
      ref={forwardedRef}
